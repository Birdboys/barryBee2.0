...
@onready var attacks = ['anticipation1']
...
func _process(delta):
	if bossAnim.current_animation == "idle" or "anticipation"  in bossAnim.current_animation:
		rotation.y = rotate_toward(rotation.y, bee_angle, deg_to_rad(bee_follow_speed)*delta)
...
func startAttack(attack: String):
	...
	match attack:
		"attack1":
			...
			#print(point1, point2, pawLeft.to_local(point1), pawLeft.to_local(point2))
